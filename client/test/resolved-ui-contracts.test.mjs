...
test("H066/H079 World Event uses authoritative impacts directly in approved banner",()=>{assert.match(ui,/worldEventOccurrences/);assert.match(ui,/chronicleEntryId/);assert.match(ui,/\.impacts/);assert.match(ui,/world-event-banner-detail/);assert.match(ui,/XEM TRONG NIÊN SỬ/);assert.doesNotMatch(ui,/event-detail-open|world-event-detail-panel|CHI TIẾT/);assert.doesNotMatch(ui,/eventName\s*===|includes\([^)]*eventName/)});
...
test("Marriage own-turn affordance remains visible but disabled with approved copy",()=>{assert.match(final,/CÓ THỂ GỬI NGOÀI LƯỢT CỦA BẠN/);assert.match(final,/btn\.disabled=!canSend/);assert.doesNotMatch(final,/!candidate\|\|!latest\.player\?\.canSendMarriage/)});
