...
function decorateExistingResidencePanel(){const old=document.querySelector<HTMLElement>(".residence-sheet");if(!old)return;old.hidden=true;if(selectedResidenceId)return;const local=player()?.currentResidenceId;if(local&&activeIds().has(local)){selectedResidenceId=local;selectedCharacterId=null;renderPanel()}}
