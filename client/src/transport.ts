...
export class GameTransport{
 ...
 constructor(){this.socket=io(undefined,{transports:["websocket","polling"]});this.socket.on("connect",()=>{this.connected=true;this.onConnection();this.onChange();this.publishSnapshot()});this.socket.on("disconnect",()=>{this.connected=false;this.onConnection();this.onChange();this.publishSnapshot()});this.socket.on("room:state",(x:RoomSnapshot)=>{this.room=x;this.onChange();this.publishSnapshot()});this.socket.on("player:state",(x:PlayerSnapshot)=>{this.player=x;this.onChange();this.publishSnapshot()})}
 publishSnapshot(){if(typeof window!=="undefined")window.dispatchEvent(new CustomEvent("ic:snapshot",{detail:{room:this.room,player:this.player}}))}
 ...
 applyState(result:Result){if(result.room)this.room=result.room;if(result.player)this.player=result.player;this.onChange();this.publishSnapshot()}
}
