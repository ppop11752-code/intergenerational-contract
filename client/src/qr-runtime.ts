...
function decorateLobby(){const host=document.querySelector<HTMLElement>(".qr-placeholder");const pin=document.querySelector<HTMLElement>(".room-pin")?.textContent?.trim()||"";if(!host||!pin)return;const url=roomJoinUrl(window.location.origin,pin);if(!url)return;const settled=host.dataset.qrReady==="1"||host.dataset.qrReady==="0";if(renderedFor===url&&settled)return;ensureStyle();renderedFor=url;host.innerHTML=`<div class="qr-functional" aria-label="Mã QR tham gia phòng"></div><button type="button" class="qr-copy">SAO CHÉP LIÊN KẾT</button><small class="qr-link-text">${url}</small>`;const box=host.querySelector<HTMLElement>(".qr-functional")!;try{if(!window.QRCode)throw new Error("renderer unavailable");new window.QRCode(box,{text:url,width:192,height:192,colorDark:"#000000",colorLight:"#ffffff",correctLevel:(window.QRCode as any).CorrectLevel?.M});host.dataset.qrReady="1"}catch{box.innerHTML=`<div class="qr-fallback">Không tạo được mã QR — hãy nhập mã phòng.</div>`;host.dataset.qrReady="0"}
