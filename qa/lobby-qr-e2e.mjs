...
io.on("connection",socket=>{
 socket.on("room:create",({playerId,displayName},ack)=>{ack({ok:true,code:"ABC123",reconnectToken:"rt-public-test"});setTimeout(()=>socket.emit("room:state",lobby(playerId,displayName)),50)});
...
try{
 ...
 const qrBox=page.locator(".qr-functional");const box=await qrBox.boundingBox();check("QR rendered at usable size",!!box&&box.width>=224&&box.height>=224,JSON.stringify(box));
 ...
 await page.evaluate(()=>Object.defineProperty(navigator,"clipboard",{value:{writeText:async()=>{throw new Error("denied")}},configurable:true}));await page.click(".qr-copy");await page.waitForTimeout(50);check("clipboard failure is non-blocking",await page.locator("#start").isEnabled()&&await page.locator(".qr-link-text").isVisible(),"start/link remain usable");
 await page.screenshot({path:path.join(outDir,"lobby-qr.png"),fullPage:true});

 const mobileCtx=await browser.newContext({viewport:{width:390,height:844}});const mobile=await mobileCtx.newPage();await mobile.goto(origin,{waitUntil:"domcontentloaded"});await mobile.click('[data-screen="create"]');await mobile.fill("#name","QR Mobile");await mobile.click("#entry-go");await mobile.waitForSelector(".lobby-screen");await mobile.waitForFunction(()=>document.querySelector('.qr-placeholder')?.getAttribute('data-qr-ready')==='1',null,{timeout:15000});const mobileBox=mobile.locator('.qr-functional');const mobileShot=await mobileBox.screenshot({path:path.join(outDir,"qr-mobile.png")});const mobilePng=PNG.sync.read(mobileShot);const mobileDecoded=decode(mobilePng);const mobileGeom=await mobile.evaluate(()=>{const el=document.querySelector('.qr-functional');const child=[...(el?.querySelectorAll('canvas,img')||[])].find(x=>{const r=x.getBoundingClientRect();return r.width>0&&r.height>0});const r=child?.getBoundingClientRect();const b=el?.getBoundingClientRect();return{moduleW:r?.width||0,moduleH:r?.height||0,boxW:b?.width||0,boxH:b?.height||0,padding:getComputedStyle(el).padding,overflow:getComputedStyle(el?.parentElement).overflow}});check("mobile QR decodes without clipping",mobileDecoded?.data===expected,`${mobileDecoded?.data||"FAIL"} ${JSON.stringify(mobileGeom)}`);check("mobile preserves 192 modules and quiet zone",mobileGeom.moduleW===192&&mobileGeom.moduleH===192&&mobileGeom.boxW>=224&&mobileGeom.boxH>=224&&parseFloat(mobileGeom.padding)>=16&&mobileGeom.overflow==="visible",JSON.stringify(mobileGeom));await mobileCtx.close();
 ...
 results.notes.push("QR decode uses jsQR on actual browser-rendered desktop and mobile screenshots. Padded decode is diagnostic only. Server harness mirrors existing ACK/state/error semantics; delayed room:state avoids test-fixture listener races.");
