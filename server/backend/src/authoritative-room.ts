...
export class AuthoritativeRoom{
  ...
  publicSnapshot(){
    ...
    return{code:this.code,started:this.started,hostPlayerId:this.hostPlayerId,initialPopulationTarget:this.initialPopulationTarget,initialNpcCount:this.initialNpcCount,founderDraw:this.founderDraw,
      ...
      game:e?{round:e.state.round,year:(e.state.round-1)*10,phase:e.phase(),ended:e.state.ended,endingReason:e.state.endingReason??null,currentTurnCharacterId:current?.id??null,currentTurnPlayerId:current?.ownerId??null,phaseDeadlineAt:this.phaseDeadlineAt,phaseDeadlineKind:this.phaseDeadlineKind,
  ...
  privateSnapshot(playerId:string){
    const e=this.engine;if(!e)return{playerId,character:null,household:null,queuePosition:null};const c=this.activeCharacterFor(playerId),h=c?e.household(c):null;
    ...
    return{playerId,character:c?{...c,ageLabel:this.ageLabel(c.ageStage)}:null,household:h,history:e.state.histories[playerId]??null,queuePosition:e.state.waitingQueue.includes(playerId)?e.state.waitingQueue.indexOf(playerId)+1:null,
      ...
      canSendMarriage:c?e.currentTurnCharacter()?.id!==c.id:false,currentPhase:e.phase(),phaseDeadlineAt:this.phaseDeadlineAt};
  }
}
