...
export class AuthoritativeRoom{
  ...
  publicSnapshot(){
    ...
    return{code:this.code,started:this.started,hostPlayerId:this.hostPlayerId,initialPopulationTarget:this.initialPopulationTarget,initialNpcCount:this.initialNpcCount,founderDraw:this.founderDraw,
      ...
      game:e?{round:e.state.round,year:(e.state.round-1)*10,phase:e.phase(),ended:e.state.ended,endingReason:e.state.endingReason??null,eventName:e.state.eventName,currentTurnCharacterId:current?.id??null,currentTurnPlayerId:current?.ownerId??null,phaseDeadlineAt:this.phaseDeadlineAt,phaseDeadlineKind:this.phaseDeadlineKind,
        ...
        historySnapshots:e.state.historySnapshots,rankings:e.rankings(),chronology:e.state.chronology.slice(-120)}:null};
  }

  privateSnapshot(playerId:string){
    const e=this.engine;if(!e)return{playerId,character:null,household:null,queuePosition:null,canInitiateBirth:false,eligibleSupportTargets:[],mandatoryQuote:null,recoveryQuotes:[],statusQuote:null};const c=this.activeCharacterFor(playerId),h=c?e.household(c):null;
    ...
    const recoveryQuotes=c&&ownsCurrentTurn&&e.phase()==="voluntary"?(["low","mid","high"] as ResourceGrade[]).map(grade=>e.recoveryQuote(grade)):[];
