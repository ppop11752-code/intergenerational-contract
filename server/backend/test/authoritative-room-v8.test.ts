...
describe("authoritative room legacy coverage",()=>{
  it("starts with 10 players and stops on a current Human decision phase",()=>{
    ...
    expect(["mandatory","status","voluntary","round_end"]).toContain(room.engine!.phase());
    expect(room.publicSnapshot().game?.population.humanControlled).toBeGreaterThan(0);
  });
